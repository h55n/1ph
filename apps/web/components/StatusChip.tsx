...
export function StatusChip({ status, className }: { status: Status; className?: string }) {
  ...
  return (
    <span
      className={cn('inline-flex items-center gap-1.5 text-xs font-mono font-medium', textColor, className)}
      role="status"
      aria-label={`Status: ${label}`}
    >
      <span className={cn('w-1.5 h-1.5 rounded-full flex-shrink-0', dotColor, pulse && 'animate-pulse-dot')} />
      {label}
    </span>
  )
}
