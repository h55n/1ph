...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const h = await prisma.hackathon
    .findUnique({ where: { slug }, select: { title: true, description: true, organizerName: true } })
    .catch(() => null)
  if (!h) return {}
  return {
  ...
  return (
    <>
      ...
      <div className="max-w-2xl mx-auto space-y-8 pt-4">
        ...
        <div>
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="ml-auto flex items-center gap-2">
              <span className="text-lg" aria-label={h.scope === 'GLOBAL' ? 'Global' : 'India'}>
                {h.scope === 'GLOBAL' ? '🌐' : '🇮🇳'}
              </span>
          ...
          <h1 className="font-serif text-3xl md:text-4xl text-text-primary leading-tight mb-4">{h.title}</h1>

          <div className="flex items-center gap-4">
            <StatusChip status={h.status} />
            {!isClosed && (
              <span className="font-mono text-sm text-text-muted">Registration closes {deadline}</span>
            )}
          </div>
        </div>
        ...
        {/* Apply CTA */}
        {isClosed ? (
          <button disabled className="w-full py-3 rounded-card bg-card border border-border text-text-muted font-mono text-sm cursor-not-allowed">
            Registration Closed
          </button>
        ) : (
          <a
        ...
        <div>
          ...
          <p className="font-sans text-text-muted text-sm leading-relaxed">{h.longDescription ?? h.description}</p>
        </div>
        ...
        <div>
          ...
          <p className="font-mono text-sm text-text-muted mt-1">
            Team size: {h.teamSizeMin}{h.teamSizeMax ? `–${h.teamSizeMax}` : '+'} members
          </p>
        </div>
        ...
        <div className="pt-4 border-t border-border flex items-center justify-between">
          ...
          <a href={`mailto:hello@1ph.dev?subject=Issue with: ${h.title}`} className="font-mono text-xs text-text-muted hover:text-accent transition-colors">
            Report an issue
          </a>
        </div>
      </div>
