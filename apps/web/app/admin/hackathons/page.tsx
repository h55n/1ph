      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          {[
            ...
            { key: 'tier', value: 'T1', label: 'T1' },
            { key: 'tier', value: 'T2', label: 'T2' },
          ].map(f => (
            <a
              key={`${f.key}-${f.value}`}
              href={`/admin/hackathons?${f.key}=${f.value}`}
              className="px-3 py-1 rounded-chip text-xs font-mono border border-border text-text-muted hover:border-accent/50 hover:text-text-primary transition-colors"
            >
              {f.label}
            </a>
          ))}
          <a href="/admin/hackathons" className="px-3 py-1 rounded-chip text-xs font-mono border border-border text-text-muted hover:border-accent/50 transition-colors">
            Clear
          </a>
        </div>
      </div>
            ...
            {hackathons.map(h => (
              <tr key={h.id} className="border-b border-border last:border-0 hover:bg-tag-bg transition-colors">
                ...
                <td className="px-3 py-2 text-xs">
                  {h.isVerified ? <span className="text-open">✓</span> : <span className="text-text-muted">—</span>}
                </td>
                <td className={`px-3 py-2 text-xs ${h.urlHealthFails > 0 ? 'text-closing' : 'text-text-muted'}`}>
                  {h.urlHealthFails > 0 ? `⚠ ${h.urlHealthFails}` : '0'}
                </td>
                <td className="px-3 py-2 text-text-muted text-xs whitespace-nowrap">
                  {h.lastSyncedAt ? new Date(h.lastSyncedAt).toLocaleDateString('en-IN') : 'Never'}
                </td>
