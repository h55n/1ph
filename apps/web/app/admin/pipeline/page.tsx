...
export default async function AdminPipelinePage() {
  ...
  // Group by source to show latest per source
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pt-4">
      ...
      {/* Latest per source */}
      <div>
        <h2 className="font-mono text-xs text-text-muted uppercase tracking-wider mb-3">Latest Status per Source</h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {Object.entries(latestBySource).map(([source, run]) => (
            <div key={source} className="bg-card border border-border rounded-card p-3">
              ...
              <div className={`font-mono text-sm font-medium ${STATUS_COLOR[run.status]}`}>
                {run.status}
              </div>
              <div className="font-mono text-xs text-text-muted mt-1">
                {new Date(run.runAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
              </div>
            </div>
          ))}
        </div>
              ...
              {runs.map((run: any) => (
                <tr key={run.id} className="border-b border-border last:border-0 hover:bg-tag-bg transition-colors">
                  ...
                  <td className="px-4 py-2">
                    <span className={`px-2 py-0.5 rounded-chip text-xs ${STATUS_COLOR[run.status]} ${STATUS_BG[run.status]}`}>
                      {run.status}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-2 text-red-400 text-xs max-w-[200px] truncate">
                    {run.errorLog ? (
                      <span title={run.errorLog ?? ''}>{run.errorLog?.slice(0, 60)}…</span>
                    ) : (
