...
const FILTERS = [
  {
    key: 'theme',
    label: 'Theme',
    options: ['AI/ML', 'Web3', 'Fintech', 'Health', 'Open', 'Hardware', 'Social Impact', 'Gaming', 'EdTech'],
  },
  {
    key: 'mode',
    label: 'Mode',
    options: ['ONLINE', 'OFFLINE', 'HYBRID'],
    display: { ONLINE: 'Online', OFFLINE: 'In-Person', HYBRID: 'Hybrid' },
  },
  {
    key: 'fee',
    label: 'Entry',
    options: ['free', 'paid'],
    display: { free: 'Free', paid: 'Paid' },
  },
  {
    key: 'team',
    label: 'Team Size',
    options: ['solo', '2-4', '5+'],
    display: { solo: 'Solo', '2-4': '2–4', '5+': '5+' },
  },
  {
    key: 'eligibility',
    label: 'Who',
    options: ['STUDENTS', 'OPEN', 'PROFESSIONALS'],
    display: { STUDENTS: 'Students', OPEN: 'Open to All', PROFESSIONALS: 'Professionals' },
  },
  {
    key: 'duration',
    label: 'Duration',
    options: ['HR24', 'HR48', 'WEEK', 'MONTH'],
    display: { HR24: '24hr', HR48: '48hr', WEEK: 'Week-long', MONTH: 'Month-long' },
  },
]
...
export function FilterBar() {
  ...
  function getDisplay(filter: typeof FILTERS[0], val: string) {
    if ('display' in filter && filter.display) {
      return (filter.display as unknown as Record<string, string>)[val] ?? val
    }
    return val
  }
        ...
        {openDropdown === 'sort' && (
          <Dropdown onClose={() => setOpenDropdown(null)}>
            {SORT_OPTIONS.map(opt => (
              <DropdownItem
                key={opt.value}
                active={(searchParams.get('sort') ?? 'prestige') === opt.value}
        ...
        return (
          <div key={filter.key} className="relative">
            ...
            {openDropdown === filter.key && (
              <Dropdown onClose={() => setOpenDropdown(null)}>
                {active && (
                  <DropdownItem active={false} onClick={() => setParam(filter.key, null)}>
                    ✕ Clear
                  </DropdownItem>
                )}
                {filter.options.map(opt => (
                  <DropdownItem
                    key={opt}
                    active={active === opt}
                    onClick={() => setParam(filter.key, opt)}
                  >
...
function DropdownItem({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        'w-full text-left px-3 py-2 text-sm font-mono transition-colors duration-100',
        active
